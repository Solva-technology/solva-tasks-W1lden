...
async def telegram_callback(payload: TelegramCallbackIn, db: AsyncSession = Depends(get_db)):
    user = await user_crud.create_or_update_telegram(db, payload.telegram_id, payload.username, payload.full_name)
    token = create_access_token(sub=user.telegram_id)
    logger.info("auth", extra={"extra": {"user_id": user.id, "action": "login_telegram"}})
