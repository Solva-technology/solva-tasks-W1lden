...
async def telegram_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    full_name = (" ".join([v for v in [frm.get("first_name"), frm.get("last_name")] if v]) or None)
    if not chat_id:
        return {"ok": True}
    user = await user_crud.create_or_update_telegram(db, chat_id, username, full_name)
    if text.strip().lower().startswith("/start"):
        await telegram_service.send_message(chat_id, "Готово. Уведомления включены.")
    return {"ok": True}
