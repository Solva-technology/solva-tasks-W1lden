...
async def create_task(payload: TaskCreateIn, db: AsyncSession = Depends(get_db), _: object = Depends(require_roles(UserRole.admin, UserRole.teacher))):
    ...
    if not group:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST)
    obj = await task_crud.create(db, payload.title, payload.description, TaskStatus.new, student, group, payload.deadline)
    if student.telegram_id:
        await telegram_service.send_message(student.telegram_id, f"Новая задача: {obj.title}")
    logger.info("task_created", extra={"extra": {"user_id": student.id, "action": "task_created", "task_id": obj.id}})
    return obj
...
async def task_detail(task_id: int, db: AsyncSession = Depends(get_db), current=Depends(get_current_user)):
...
async def patch_task(task_id: int, payload: TaskPatchIn, db: AsyncSession = Depends(get_db), current=Depends(get_current_user)):
    obj = await task_crud.get(db, task_id)
    if not obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    if current.role in {UserRole.admin, UserRole.teacher}:
        ...
        if payload.status is not None:
            ...
            if updated.group.manager and updated.group.manager.telegram_id:
                await telegram_service.send_message(updated.group.manager.telegram_id, f"Статус задачи {updated.id}: {old_status} → {updated.status}")
            logger.info("task_status_changed", extra={"extra": {"user_id": current.id, "action": "task_status_changed", "task_id": updated.id, "old_status": str(old_status), "new_status": str(updated.status)}})
        else:
            await db.commit()
